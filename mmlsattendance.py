...
async def load_online(SubjectDB_obj, user_id, password, *, semaphore=None):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f"mmls.mmu.edu.my/: {resp.status}")
        ...
        print(
            f"mmls.mmu.edu.my/checklogin: {resp.status}")
        if resp.status == 500:
            return False
        # ===== Parse subjects =====
        tree = etree.parse(StringIO(await resp.text()), etree.HTMLParser())
        ...
        names = [name.split(' - ')
                 for name in tree.xpath(f"{SUBJECT_XPATH}/text()")]
        links = [link[24:].split(':')
                 for link in tree.xpath(f"{SUBJECT_XPATH}/@href")]
        ...
        print(f"mmls.mmu.edu.my/logout: {resp.status}")
        return True
        ...
        while tasks:
            ...
            if class_id in class_id_to_class_obj:
                ...
                scraped_timetable = ScrapedTimetable(
                    timetable_id=int(tree.xpath(
                        "//input[@name='timetable_id']/@value")[0]),
                    start_time=tree.xpath(
                        "//input[@name='starttime']/@value")[0][:-3],
                    end_time=tree.xpath(
                        "//input[@name='endtime']/@value")[0][:-3],
                    class_date=tree.xpath(
                        "//input[@name='class_date']/@value")[0],
                    class_id=class_id,
        ...
        while start_ttid is None or end_ttid is None:
            if start_ttid is None:
                start_ttid_task = asyncio.create_task(
                    date_to_timetable(
                        start_date, 1, session=session, semaphore=semaphore)
                )
            if end_ttid is None:
                end_ttid_task = asyncio.create_task(
                    date_to_timetable(
                        end_date, -1, session=session, semaphore=semaphore)
                )
    ...
    async with semaphore:
        while(True):  # Option: 1 for first occurence, -1 for last occurence.
            ...
            if resp.status == 500:
                ...
                continue
            tree = etree.parse(StringIO(await resp.text()), etree.HTMLParser())
            curr_date = date.fromisoformat(tree.xpath(
                "//input[@name='class_date']/@value")[0])
...
# prototype func(please check for error)
async def sign_now(url, classid, date_register, starttime_register, endtime_register, ttid, student_id, student_password, semaphore=None):
    semaphore = semaphore or asyncio.Semaphore(MAX_CONCURRENT_REQUESTS)
    async with aiohttp.ClientSession() as session:
...
async def checkmmls_link(url, SubjectDB_obj):
    semaphore = asyncio.Semaphore(MAX_CONCURRENT_REQUESTS)
    async with aiohttp.ClientSession() as session:
        try:
            client = await _request('GET', url, session=session, semaphore=semaphore)
            ...
            reference_head_tree = etree.parse(StringIO(await reference_head.text()), etree.HTMLParser())
            reference_head = reference_head_tree.xpath("//head/text()")
            if head_tag == reference_head:
                class_id_to_class_obj = {
                    kelas.id: kelas for kelas in SubjectDB_obj.selected_classes}
                class_id = int(tree.xpath(
                    "//input[@name='class_id']/@value")[0])
                if class_id in class_id_to_class_obj:
                    kelas = class_id_to_class_obj[class_id]
                    scraped_timetable = ScrapedTimetable(
                        timetable_id=int(tree.xpath(
...
class SubjectDB:
    ...
    def add_subject(self, sid, *, code=None, name=None, coordinator_id=None):
        ...
        t_subject = self.Subject(self, sid, code=code,
                                 name=name, coordinator_id=coordinator_id)
