...
async def login(ctx, studentid=1181101213, password=None):  # change here
    if ctx.author.id in discordid_to_subjectdatabase:
        ...
        for wrongpass in range(3):
            ...
            if password.lower() == "cancel":
                ...
                await dm_channel.send("Let me access the MMLS to log you in... 👨‍💻")
        ...
        try:
            if attendance_startdate is None:
                attendance_startdate = attendance_enddate = (
                    date.fromisoformat(attendance_enddate)
                )
...
async def sign(ctx, attendance_date_or_url=None):  # here???
    if ctx.author.id in discordid_to_subjectdatabase:
        ...
        if not found_dates:
            ...
            await ctx.send(f"{ctx.author.mention} force-sign-in these/this attendance(s)? __{printAttendance_date}__")

            # def check(reaction, user):
            #    return user == ctx.message.author and str(reaction.emoji == '✅')

            try:
                await bot.wait_for('reaction_add', timeout=500, check=lambda reaction, user: reaction.emoji == '✅' and user == ctx.message.author)
            except asyncio.TimeoutError:
                ...
                async with ctx.typing():
                    ...
                    endtime_register = (endtime_register.time()).isoformat(
                        timespec="seconds")
                    attendance_register = (datetime.now()).date()
                    with StringIO() as file:
                        await force_sign_attendance(ctx, SubjectDB_obj, attendance_date_or_url, attendance_date_or_url, attendance_register, starttime_register, endtime_register, mmuid, mmupassword, file)
...
bot.run("NzcwOTg0MTUwNzYwMDMwMjE4.X5lg8Q.xSNyjz4PCZ5iI273ZGnci2dbxak")
